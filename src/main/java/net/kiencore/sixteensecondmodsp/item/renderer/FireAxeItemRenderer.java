package net.kiencore.sixteensecondmodsp.item.renderer;
...
import net.kiencore.sixteensecondmodsp.utils.AnimUtils;
import net.kiencore.sixteensecondmodsp.item.model.FireAxeItemModel;
import net.kiencore.sixteensecondmodsp.item.FireAxeItem;
import net.kiencore.sixteensecondmodsp.interfaces.RendersPlayerArms;
...
public class FireAxeItemRenderer extends GeoItemRenderer<FireAxeItem> implements RendersPlayerArms {
	public FireAxeItemRenderer() {
		super(new FireAxeItemModel());
	}

	@Override
	public RenderType getRenderType(FireAxeItem animatable, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, VertexConsumer buffer, int packedLight, ResourceLocation texture) {
		return RenderType.entityTranslucent(getTextureLocation(animatable));
	}

	static {
		AnimationController.addModelFetcher(animatable -> {
			if (animatable instanceof FireAxeItem) {
				Item item = (Item) animatable;
				BlockEntityWithoutLevelRenderer ister = new FireAxeItemRenderer();
	...
	protected FireAxeItem animatable;
	...
	@Override
	public void render(GeoModel model, FireAxeItem animatable, float partialTicks, RenderType type, PoseStack matrixStackIn, MultiBufferSource renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red,
	...
	@Override
	public void render(FireAxeItem animatable, PoseStack stack, MultiBufferSource bufferIn, int packedLightIn, ItemStack itemStack) {
	...
	public void renderRecursively(GeoBone bone, PoseStack stack, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
		...
		if (name.equals("riglefthand") || name.equals("rigrighthand")) {
			bone.setHidden(true);
			renderingArms = true;
		} else {
		...
		if (this.transformType.firstPerson() && renderingArms) {
			...
			} else if (name.equals("rigrighthand")) {
				stack.translate(1.0f * SCALE_RECIPROCAL, 2.0f * SCALE_RECIPROCAL, 0.0f);
	...
	@Override
	public ResourceLocation getTextureLocation(FireAxeItem instance) {
		return super.getTextureLocation(instance);
	}

	@Override
	public Integer getUniqueID(FireAxeItem animatable) {
